...
class PolypGenDataset(Dataset):
    def __init__(
        ...
        data_path = os.path.join(
            self.data_dir, "PolypGen2021_MultiCenterData_v3"
        )
        ...
        for i in range(1, 7):
            image_dir = os.path.join(data_path, f"data_C{i}", f"images_C{i}")
            # mask_dir = os.path.join(data_path, f"data_C{i}", f"masks_C{i}")

            image_paths = sorted(glob.glob(os.path.join(image_dir, "*.jpg")))
            mask_paths = [
                im_path.replace("images_", "masks_")
                .replace(".jpg", "_mask.jpg")
                .replace("]", "")
                for im_path in image_paths
            ]

            X_train_tmp, X_val_test_tmp, y_train_tmp, y_val_test_tmp = (
                train_test_split(
                    image_paths,
                    mask_paths,
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        assert (image_tensor.shape[1], image_tensor.shape[2]) == (
            mask_tensor.shape[0],
            mask_tensor.shape[1],
        ), f"Image H, W ({(image_tensor.shape[1], image_tensor.shape[2])}) and mask H, W ({(mask_tensor.shape[0], mask_tensor.shape[1])}) mismatch. Image: {self.image_paths[index]}, mask: {self.mask_paths[index]}"
...
if __name__ == "__main__":
    train_dataset = PolypGenDataset("./data/", mode="train")
    val_dataset = PolypGenDataset("./data/", mode="val")
    test_dataset = PolypGenDataset("./data/", mode="test")
