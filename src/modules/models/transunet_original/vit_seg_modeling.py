...
class Block(nn.Module):
    ...
    def load_from(self, weights, n_block):
        ...
        with torch.no_grad():
            ...
            query_bias = np2th(weights[pjoin(ROOT, ATTENTION_Q, "bias")]).view(
                -1
            )
            key_bias = np2th(weights[pjoin(ROOT, ATTENTION_K, "bias")]).view(
                -1
            )
            value_bias = np2th(weights[pjoin(ROOT, ATTENTION_V, "bias")]).view(
                -1
            )
            out_bias = np2th(weights[pjoin(ROOT, ATTENTION_OUT, "bias")]).view(
                -1
            )
...
if __name__ == "__main__":
    ...
    model = VisionTransformer(
        vit_name, img_size=384, num_classes=1, zero_head=False, vis=False
    )
    print(model)  # for testing purposes only
    # model.load_from(weights)  # Uncomment to load weights if available
    input_tensor = torch.randn(2, 3, 384, 384)
    print(model(input_tensor).shape)  # Should print the output shape
