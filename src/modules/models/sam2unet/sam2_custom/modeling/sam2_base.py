...
class SAM2Base(torch.nn.Module):
    ...
    def _use_mask_as_output(
        ...
        """Directly turn binary `mask_inputs` into a output mask logits without
        using SAM.
    ...
    def _encode_new_memory(
        ...
        """Encode the current image and its prediction into a memory
        feature."""
        B = current_vision_feats[-1].size(1)  # batch size on this frame
        C = self.hidden_dim
        H, W = feat_sizes[-1]  # top-level (lowest-resolution) feature size
    ...
    def _apply_non_overlapping_constraints(self, pred_masks):
        """Apply non-overlapping constraints to the object scores in
        pred_masks.

        Here we keep only the highest scoring object at each spatial location
        in pred_masks.
        """
