...
class ASPP(nn.Module):
    def __init__(self, in_c, out_c, rate=[1, 6, 12, 18]):
        ...
        self.c1 = nn.Sequential(
            nn.Conv2d(
                in_c, out_c, kernel_size=3, dilation=rate[0], padding=rate[0]
            ),
            nn.BatchNorm2d(out_c),
        )
        ...
        self.c2 = nn.Sequential(
            nn.Conv2d(
                in_c, out_c, kernel_size=3, dilation=rate[1], padding=rate[1]
            ),
            nn.BatchNorm2d(out_c),
        )
        ...
        self.c3 = nn.Sequential(
            nn.Conv2d(
                in_c, out_c, kernel_size=3, dilation=rate[2], padding=rate[2]
            ),
            nn.BatchNorm2d(out_c),
        )
        ...
        self.c4 = nn.Sequential(
            nn.Conv2d(
                in_c, out_c, kernel_size=3, dilation=rate[3], padding=rate[3]
            ),
            nn.BatchNorm2d(out_c),
        )
...
class ResUnetPlusPlus(nn.Module):
    def __init__(self, in_channels: int = 3, n_classes: int = 1):
        ...
        self.output = nn.Conv2d(16, n_classes, kernel_size=1, padding=0)
